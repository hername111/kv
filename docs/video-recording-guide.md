...
| 1:52-2:15 | 切到测试终端，先框出 Rust 测试通过，再框出 `87 passed` | “除了界面演示，我们还有 crate 单元测试、集成测试和 87 项真实 TCP 协议测试，覆盖 DDL、DML、JOIN、事务、错误处理，以及关闭服务后重新打开数据库的持久化恢复；Clippy 以 warnings as errors 通过。” | 功能完善、代码规范、测试充分 |
| 2:15-2:40 | 展示 `docs/open-source-references.md` 的参考表和差异表，停在 SQLite/PostgreSQL/InnoDB/BusTub 四行 | “设计思想参考了 SQLite 文件格式、PostgreSQL 槽页、InnoDB 索引和 BusTub 分层，但没有复制源码。我们的改进包括带魔数和版本的 superblock、可跨重启恢复的 freelist，以及越界和链表环检测；同时明确没有 WAL 等生产能力。” | 开源引用、区别、改进、诚实边界 |
