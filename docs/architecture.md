...
API 使用结构化 JSON 解析，请求体上限为 64 KiB，并返回禁止缓存和 MIME 嗅探响应头。
...
- MySQL TCP 层支持分片数据包重组，并拒绝超过 8 MiB 或中途截断的数据包。
- B+Tree 与 Slotted Page 在解码时检查长度、偏移和节点类型，损坏数据返回错误而非触发 panic。
- 页面释放时同步失效缓冲池缓存，防止页号复用后读到旧数据。

## 当前边界

- 没有 WAL，因此进程在多页更新中途崩溃时不保证原子恢复。
- B+Tree 使用固定阶数，超出单页容量时会拒绝写入，尚未实现按字节利用率分裂和完整删除再平衡。
...
2. 让 B+Tree 按字节占用提前分裂，并实现删除后的合并与再平衡。
