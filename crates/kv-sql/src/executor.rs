        ...
        match plan {
                ...
                for (_, val) in &all {
                    if let Ok(row) = kv_storage::codec::deserialize_row(val) {
                        let include = match &filter {
                            Some(pred) => {
                                pred.evaluate(&row, &col_names) == Some(Value::Bool(true))
                            }
                            None => true,
                        };
                        if include {
                ...
                for (key, val) in &all {
                    ...
                    if matches {
                        for (col, expr) in &sets {
                            if let Some(idx) =
                                col_names.iter().position(|c| c.eq_ignore_ascii_case(col))
                            {
                                let new_val =
                                    expr.evaluate(&row, &col_names).unwrap_or(Value::Null);
                        ...
                        if txn_id > 0 {
                            ...
                            self.storage
                                .put(meta.table_id, key, &new_val, txn_id)
                                .await?;
                        }
                        count += 1;
                    }
            ...
            PlanNode::CreateIndex {
                ...
                if let Some(ks) = self
                    .storage
                    .as_any()
                    .downcast_ref::<kv_storage::KvStorage>()
                {
            ...
            PlanNode::IndexScan {
                ...
                let pk_list = self
                    .storage
                    .index_lookup(index_id, key.as_bytes(), txn_id)
                    .await?;
                let mut rows = Vec::new();
                for pk in &pk_list {
            ...
            PlanNode::CommitTransaction => {
                let txn_id = session
                    .txn_id()
                    .ok_or_else(|| KvError::Internal("no active transaction".to_string()))?;
                {
            ...
            PlanNode::Sort { source, order_by } => {
                ...
                inner.rows.sort_by(|a, b| {
                    for ob in &order_by {
                        let idx = col_names
                            .iter()
                            .position(|c| c.eq_ignore_ascii_case(&ob.column));
                        if let Some(pos) = idx {
            ...
            PlanNode::Join { left, right, join } => {
                ...
                let left_cols: Vec<String> =
                    left_rs.columns.iter().map(|c| c.name.clone()).collect();
                let right_cols: Vec<String> =
                    right_rs.columns.iter().map(|c| c.name.clone()).collect();
                let left_idx = left_cols
                    .iter()
                    .position(|c| c.eq_ignore_ascii_case(&join.on.0));
                let right_idx = right_cols
                    .iter()
                    .position(|c| c.eq_ignore_ascii_case(&join.on.1));
                let mut rows = Vec::new();
    ...
    fn encode_key(&self, row: &Row, meta: &TableMeta) -> KvResult<Vec<u8>> {
        ...
        if pk_idx >= row.values.len() {
            return Err(KvError::Internal(
                "primary key index out of range".to_string(),
            ));
        }
