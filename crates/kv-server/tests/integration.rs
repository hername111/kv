...
impl kv_common::traits::StorageEngine for MockStorage {
    ...
    async fn put(&self, tid: u64, key: &[u8], value: &[u8], _txn: u64) -> kv_common::error::KvResult<u64> {
        self.data.lock().unwrap().entry(tid).or_default().push((key.to_vec(), value.to_vec()));
        Ok(1)
    }
    async fn get(
        &self, tid: u64, key: &[u8], _txn: u64,
    ) -> kv_common::error::KvResult<Option<Vec<u8>>> {
        Ok(self.data.lock().unwrap().get(&tid).and_then(|v| {
            v.iter().find(|(k, _)| k == key).map(|(_, v)| v.clone())
        }))
    }
    async fn scan(
        &self, tid: u64, _s: &[u8], _e: &[u8], _txn: u64,
    ...
    async fn create_index(&self, _t: u64, _c: u64) -> kv_common::error::KvResult<u64> { Ok(1) }
    async fn index_lookup(
        &self, _i: u64, _k: &[u8], _txn: u64,
    ) -> kv_common::error::KvResult<Vec<Vec<u8>>> { Ok(vec![]) }
...
async fn test_insert_select_flow() {
    ...
    executor.execute_sql("CREATE TABLE t (id INT PRIMARY KEY)", &session).await.unwrap();
...
async fn test_update_flow() {
    ...
    executor.execute_sql("CREATE TABLE t (id INT PRIMARY KEY, val INT)", &session).await.unwrap();
