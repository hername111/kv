...
本项目在设计阶段查阅了多个成熟开源数据库和教学数据库项目的公开资料，参考对象主要是文件组织、页面布局、B+Tree 结构和缓冲池职责划分。项目实现采用独立代码完成，相关引用在 README、docs 文档和本报告中标明项目名称、固定版本或提交、许可证、官方链接、参考内容以及本项目对应实现文件。主要参考来源见表~\ref{tab:opensource}。
...
\toprule
参考项目 & 固定版本与许可证 & 借鉴内容与本项目对应实现\\
\midrule
SQLite & 3.50.4，Public Domain & 参考固定页、文件头和 free-list 空闲页管理；本项目在 \code{pager.rs} 中实现更小的 superblock、空闲页单链表和文件校验。\\
PostgreSQL & REL\_18\_0，PostgreSQL License & 参考 slotted page 页面布局；本项目在 \code{page.rs} 中实现页头、槽目录、tuple 数据区和布局校验。\\
MySQL InnoDB & mysql-8.4.0，GPLv2 & 参考 B+Tree 索引页与叶节点组织方式；本项目在 \code{btree.rs} 中实现内部节点、叶节点、分裂和叶链扫描。\\
CMU BusTub & 固定 commit，MIT & 参考教学数据库分层与 BufferPoolManager 职责；本项目在 \code{buffer.rs} 和 \code{Pager} trait 中实现 LRU 缓冲池抽象。\\
\bottomrule
\end{tabularx}
\end{table}
...
维度 & 参考系统做法 & 本项目区别与改进\\
\midrule
文件与空闲页 & SQLite 使用成熟文件头、page 1 free-list trunk/leaf 结构，并配合 journal/WAL。 & 本项目使用更小的 superblock 和空闲页单链表，额外加入魔数、版本、截断、页号范围和链表环检测，并用跨重启测试验证页面复用。\\
数据页 & PostgreSQL slotted page 保存 LSN、checksum、special space、line pointer 等生产级字段。 & 本项目保留教学所需页头、槽项和 tuple 数据区，集中校验槽目录和 tuple 边界；WAL 页头和复杂恢复信息作为后续扩展内容。\\
索引结构 & InnoDB 支持聚簇索引、二级索引、高并发访问和恢复机制。 & 本项目实现固定阶 B+Tree、叶节点链表扫描和超页拒绝；降低复杂度以突出 B+Tree 原理，并明确不等同于 InnoDB 的生产级索引。\\
缓冲池 & BusTub 使用 page guard、pin count、锁和 replacer 管理页面生命周期。 & 本项目用 Rust \code{Pager} trait 拆分读写、分配、释放和刷盘职责，用 \code{Mutex} 与 LRU 管理缓存，并增加释放页后缓存失效测试。\\
入口与观察性 & 成熟数据库主要面向完整客户端生态，BusTub 主要面向课程测试。 & 本项目同时提供 MySQL 协议和 Web 工作台，Web 界面展示真实后端状态、schema、索引数量和执行耗时，更适合课程演示与调试。\\
\bottomrule
\end{tabularx}
\normalsize
\end{table}
...
通过对比可以看出，本项目主要吸收数据结构、模块边界和工程经验，并结合课程目标形成了以下特点：

\begin{enumerate}[label=\arabic*.]
\item \textbf{Rust 原生实现。}利用所有权、trait、\code{Result} 和线程安全类型构建内存安全的数据库内核。
\item \textbf{链路完整。}从 SQL 词法解析到磁盘页面持久化均有自研实现，能够展示数据库内部关键环节。
\item \textbf{边界检查充分。}对文件格式、页面边界、协议分片、HTTP 请求大小、缓存失效和重启恢复均有测试覆盖，体现了对异常输入和演示复现的考虑。
\item \textbf{双入口可观察。}MySQL 协议适合标准客户端验证，Web 工作台适合课堂展示，脚本还能读取数据库页信息和 Web 当前状态。
\item \textbf{范围明确。}报告说明 WAL、完整崩溃恢复、行级锁和完整 MySQL 兼容性属于后续扩展方向，项目定位保持在教学型数据库系统原型。
\end{enumerate}

因此，本项目在课程范围内完成了可运行、可测试、可解释的数据库系统原型。相对于参考项目，它的价值不在于功能规模，而在于把数据库核心机制压缩到一个可以学习、复现和演示的工程闭环中，并通过测试、脚本和界面展示证明实现结果。
